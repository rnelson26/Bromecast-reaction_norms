data {
  ...
  array[n_train] int<lower=0> plot_index_train; /// 1. why <lower=0> should it be 1 (not sure if it matters)

  vector[n_test] neighbors_test;
  vector[n_test] annual_test;
  vector[n_test] perennial_test;
  vector[n_test] shrub_test;
  array[n_test] int<lower=0> plot_index_test; /// same comment 
...
array[n_test] int<lower=n_site_year_train + 1, upper=n_site_year> site_year_id_test; // Test set
  ...
  /// 3. if p_X > s_X then we probably also need q_X > q_Soil
...
parameters {
  ...
  /// 2. you could try pi()/2 like you do for u_zeta_0 below. It could be fine and will look cleaner 
  /// Sometimes behavior near pi/2 is poor since tan(pi/2)=Inf 
  /// and it's safe to do pi()/2-1e-6 (some number just a little less than pi). 
  /// I think that is why I hard coded the number pi originally 
  ...
  /// 3 (continued). So n_X, the number of rows, corresponds to site, correct? 
  /// at every single site we have measurements of both climte and soil. 
  /// what are the number of PCs, q_X, identical, just coincidence? 
  /// from what I understand, we have a LOT more climate than soil variables 
  /// so wouldn't we need more PCs for explaining climate than soil? 
  /// I could see something like the numbers below for example 
  /// matrix[n_X, 5] W;
  /// matrix[n_X, 2] W_soil;
  matrix[n_X, q_X] W;
...
/// 4. This is fine, but you could also do beta_0 ~ normal(mu_beta_0, zeta_0^2*K)
/// then mu_beta_0 would be global mean across all genotypes and beta_0 is an uncentered genotype random effect
/// equivalent statistically, but since this is working just keep as is. 
...
transformed parameters {
  ...
  /// 5. are you wanting to induce a half-cauchy(0, 5) prior for zeta_0?
  /// this is less informative than zeta_0 ~ half-cauchy(0, 1).
  /// if that is what you want great, and goes along with backing off some of the prior like we talked about
...
for (l in 1:q_X)
  ...
  for (l in 1:q_X) {
   ...
   /// 6. continued same comment for zeta_soil remove sqrt()
   for (l in 1:q_X) {
...
for (l in 1:q_X) {
  ...
  mu_beta[l] ~ normal(0, 100); /// 7. vague normal priors. Let's use these unless having convergence issues 
}
for (l in 1:q_X) {
...
/// 8. don't need these, when you define a parameter in the "parameters" block, the default prior is uniform 
...
/// 8. yeah you could probably make some of these prior more vague 
...
/// 9. normal_lpdf takes mean and sd so these are good! 
...
/// 10. this may take some time since you are predicting fitness for each individual plant. 
/// if run time is an issue for debugging you could silence generated quantities  
