...
parameters {
  ...
  real<lower=0> theta; // Dispersion parameter for the negative binomial
...
generated quantities {
  ...
  // Training predictions with fitted random effects
  for (i in 1:n_train) {
    int idx = idx_plant_train[i];
    int idx_site = idx_plant_train_site[i];
    ...
    if (plot_index_train[i] != 0)
      mu_base += eta_plot_centered[plot_index_train[i]];
    if (transect_index_train[i] != 0)
      mu_base += eta_transect_centered[transect_index_train[i]];
...
for (i in 1:n_test) {
  ...
  mu_test[i] = inv_logit(logit_p);
  y_test_pred[i] = bernoulli_logit_rng(logit_p);
  y_test_pred2[i] = bernoulli_logit_rng(logit_p);
 }
}
