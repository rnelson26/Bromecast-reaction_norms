...
data {
  ...
  // Toggle model components on/off to run different versions of model 
int<lower=0, upper=1> use_genetic;
int<lower=0, upper=1> use_inter_competition;
int<lower=0, upper=1> use_intra_competition;
int<lower=0, upper=1> use_climate;
}
...
if (use_climate == 1) {
  logit_p += dot_product(W[idx], beta[idx_genotype])
...
if (use_intra_competition == 1) {
  logit_p += beta_neighbors * neighbors_train[i];
}

if (use_inter_competition == 1) {
  logit_p += beta_annual * annual_train[i]
           + beta_perennial * perennial_train[i]
           + beta_shrub * shrub_train[i];
}

logit_p += site_year_effect_train_scaled_centered[site_year_id_train[i]];
...
generated quantities {
  ...
  for (i in 1:n_train) {
    ...
    if (use_climate == 1)
      logit_p += dot_product(W_full[idx], beta[g]) + dot_product(W_soil_full[idx_site], beta[g]);

    if (use_genetic == 1)
      logit_p += beta_0_centered[g];
    ...
    logit_p += site_year_effect_train_scaled_centered[s];
    ...
    if (use_climate == 1)
      mu_fixed += dot_product(W_full[idx], beta[g]) + dot_product(W_soil_full[idx_site], beta[g]);

    if (use_intra_competition == 1)
      mu_fixed += beta_neighbors * neighbors_train[i];

    if (use_inter_competition == 1)
      mu_fixed += beta_annual * annual_train[i]
                + beta_perennial * perennial_train[i]
                + beta_shrub * shrub_train[i];

    p_train_fixed[i] = inv_logit(mu_fixed);
    r_train_pred_fixed[i] = bernoulli_logit_rng(mu_fixed);
  }
  ...
  for (i in 1:n_test) {
    ...
    if (use_intra_competition == 1)
      logit_p += beta_neighbors * neighbors_test[i];

    if (use_inter_competition == 1)
      logit_p += beta_annual * annual_test[i]
               + beta_perennial * perennial_test[i]
               + beta_shrub * shrub_test[i];

    logit_p += site_year_noise;
  ...
  for (i in 1:n_train_full) {
    ...
    if (use_intra_competition == 1)
      logit_p += beta_neighbors * neighbors_train_full[i];

    if (use_inter_competition == 1)
      logit_p += beta_annual * annual_train_full[i]
               + beta_perennial * perennial_train_full[i]
               + beta_shrub * shrub_train_full[i];

    if (s <= n_site_year_train)
      logit_p += site_year_effect_train_scaled_centered[s];
    else
      logit_p += normal_rng(0, sigma_site_year);
  ...
  for (i in 1:n_test_full) {
    ...
    if (use_intra_competition == 1)
      logit_p += beta_neighbors * neighbors_test_full[i];

    if (use_inter_competition == 1)
      logit_p += beta_annual * annual_test_full[i]
