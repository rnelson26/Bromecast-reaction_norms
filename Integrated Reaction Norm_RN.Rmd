...
bioclim <- bioclim %>%  filter(Emerged == "Y", Reproduced == "Y") %>%  
  filter(!is.na(Fecundity)) %>% filter(Fecundity > 0) #some that have zero seeds otherwise included bc coded as reproduced 

idx_sites <- which(bioclim$site %in% unique_sites)
idx_sites <- sapply(unique(bioclim$site), function(site) which(bioclim$site == site)[1])
...
idx_plant <- as.numeric(as.factor(df$site)) 
