functions {
  ...
  // JUSTIN COMMENT: I think there are some issues with this function. Let's try the rejection sampler below 
...
//  }
  ...
  // JUSTIN COMMENT: This rejection sampler could be slow if mu is really small, but I don't think that will be an issue for us 
  int ztnb_rng(real mu, real theta) {
    int y = 0;
    while (y == 0) {
...
//this does use a rejection-sampling style cumulative sum for generating posterior predictive draws in generated quantities
...
parameters {
 ...
 vector<lower=0, upper=1.57079632679>[q_X_soil] u_zeta_soil;
  ...
  real beta_perennial;
  real beta_shrub;
  // JUSTIN COMMENT: You could probably drop the upper=10000 because you set a fairly informative gamma prior below. 
  real<lower=0> theta;
...
model {
  ...
  // JUSTIN COMMENT: 
  // You're using mu_beta_soil instead of beta_soil here and everywhere else. That means no GxE for soil but we do have GxE for climate
  // it that what you want? If so, that's fine, I would just delete the ghost parameters zeta_soil, beta_soil, and beta_soil_raw. 
  // because stan will still sample which could slow run time they aren't used in the model 
...
// JUSTIN COMMENT: I believe site_year_effect_train_raw and eta_plot_raw should be normal(0, 1) so that 
// site_year_effect_train_raw * sigma_site_year ~ normal(0, sigma_site_year), where I am using normal(mean, sd). 
// Same thing for eta_plot_raw * sigma_plot ~ normal(0, sigma_plot)
// LASTLY, you mentioned you're having some overflow/computational issues
// We could try reverting back to stronger priors. Try 100 --> 1.5 in all places, for example, sigma_site_year ~ normal(0, 1.5)
// and alpha ~ normal(0, 1.5)
...
generated quantities {
  // JUSTIN COMMENT: this is a little hacky and my actually be too constraining 
  // (i.e., don't some plants have fitness greater than exp(10)~=22,000?)
  // I assume you did the fmin() below based on those overflow issues.
  // Hopefully stronger priors and the proposed rejection sampler will take care of those issues. 
 // real mu_cap = 10;
...
// JUSTIN COMMENT, y_test_pred_fixed is never used 
...
array[n_train_full] int y_train_pred_full_fixed;
  ...
  // Test
    ...
    // JUSTIN COMMENT, I'm confused by this. Aren't all plot_index_test == 0? 
    // These pertain to satellite sites, which don't have plots
    //RAN: this is an if else statement that assigns no plot to the satellite site and a plot random effect to the common garden sites 
    real mu_final = mu_base + (plot_index_test[i] == 0 ? 0 : eta_plot_centered[plot_index_test[i]]);
    mu_test[i] = exp(mu_final);
