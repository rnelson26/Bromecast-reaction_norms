...
model {
  ...
  for (i in 1:n_X) {
    for (j in 1:p_X) {
      target += normal_lpdf(
        X[i, j] | dot_product(Lambda[j, ], W[i, ]), sigma[j]
      );
    }
  }
  ...
  for (i in 1:n_X_soil) {
    for (j in 1:s_X) {
      target += normal_lpdf(
        X_soil[i, j] | dot_product(Lambda_soil[j, ], W_soil[i, ]), sigma_soil[j]
      );
    }
  }
}
...
generated quantities {
  ...
  for (i in 1:n_train) {
    ...
    real mu_final =
      mu_base
      + (plot_index_train[i] == 0 ? 0 : eta_plot_centered[plot_index_train[i]]);
  ...
  for (i in 1:n_test) {
    ...
    real mu_final =
      mu_base
      + (plot_index_test[i] == 0 ? 0 : eta_plot_centered[plot_index_test[i]]);
